    ...
    public function getFormatAll(){
        ...
        if (isset($order_column)) {
            ...
            switch ($i) {
                ...
                case 6;$orderSql = " uptime " . $order_dir;break;
                default;$orderSql = '';
            }
        }
        ...
        {
            $add='';
            if(strpos(getUserCzqx(),"添加展会")>-1){
                $add="<a class='btn btn-xs add' czid=".$value['id'].">Duplicate</a>";
            }
            $tb_row=array(
                $key+1,
                "<input type=checkbox name=pid[] value=".$value['id'].">",
                '<a href=/CRM/index.php/Home/Project/view?czid='.$value['id'].' target=_blank>'.$value['title'].'</a>',
                $classmodel->getNameById($value['hangye']),
                $classmodel->getNameById($value['s1']) .'-'.$classmodel->getNameById($value['s2']),
