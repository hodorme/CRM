  ...
  <table id="ListTbl" class="table table-striped table-bordered hover">
    <thead class="bg-info">
      <tr>
        ...
        <th>所属部门</th>
        <th>角色名称</th>
        <th width="400">模块权限</th>
        <th width="400">操作权限</th>
        <th>更新时间</th>
        <th>ID</th>
      </tr>
    </thead>
  ...
  <form action='' method='post' id="addForm">
    <table class='table table-striped table-bordered'>
      <tr>
        ...
        <td><input id="classname" name="classname" type="text" class="form-control" placeholder="填写角色名称"></td>
        <td width="150">所属部门</td>
        <td><select id='bmid' name='bmid' class='form-control'>
            <option value=''>所属部门</option>
            <volist name=":ShowList('Bumen','parid=0','id')" id="ft">
        ...
        <td>显示排序</td>
        <td><input id="sequence" name="sequence" type="text" class="form-control" placeholder="填写显示排序"></td>
      </tr>
        ...
        <td colspan="5">
            <table class="table table-striped table-bordered">
              <volist name=":ShowList('sys_menu','sid='.$ft['id'],'seq')" id="sd">
                <tr>
              <td width="120"><label><input value='{$sd.id}' id="qx{$sd.id}" name="qx[]" fid="{$ft.id}" class="sd{$ft.id}" type="checkbox"> {$sd.title}</label>
              </td>
              ...
              <td style="text-align: left; padding-left: 20px;">
                <volist name=":ShowList('Quanxian','sid='.$sd['id'],'id')" id="td">
                  <php>$str=(explode("@",$td['qx']));</php>
                  <foreach name="str" item="vo">
                    <label> <input value='{$vo}' id="czqx{$td.id}" name="czqx[]" fid="{$ft.id}" class="td{$ft.id}" type="checkbox"> {$vo} </label>
                  </foreach>
                </volist>
              </td>
                </tr>
          ...
          <volist name=":ShowList('Quanxian','sid='.$ft['id'],'id')" id="td">
            <php>$str=(explode("@",$td['qx']));</php>
            <foreach name="str" item="vo">
              <label> <input value='{$vo}' id="czqx{$td.id}" name="czqx[]" fid="{$ft.id}" class="td{$ft.id}" type="checkbox"> {$vo} </label>
            </foreach>
          ...
          $.ajax({
            ...
            success: function (json) {
              $.each(json, function (i, item) {
                ...
                if(item.czqx){
                  var arr1 = item.czqx;
                  $("input[name='czqx[]']").each(function(){
                    var aaa1=$(this).val();
                   // alert(aaa1)
                    //if(arr1.indexOf(aaa1)>0){
                    if(arr1.indexOf(aaa1)>=0){
                  ...
                  $("input[name='czqx[]']").prop("checked",false);
                }
              });
            }
          });
