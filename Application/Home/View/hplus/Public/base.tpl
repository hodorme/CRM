...
<link href='__CSS1__/ww2.0.css' rel='stylesheet' type='text/css' />
    ...
    $(document).on("click",".SelectAllId",function(){
        ...
        if($(this).is(':checked')){
            ...
            if($("input[type='checkbox']").get(0).checked){
                var len = $("input[type='checkbox']:checked:gt(0)").length;
                    }else{
            var len = $("input[type='checkbox']:checked").length;
        }
            $('#checkboxlen').text(len+' accounts are selected');
        }else{
...
//                url: "/index.php/Home/Index/index?show=searchCountry",
                ...
                url:"{:U('Index/index',array('show' => 'searchCountry'))}",
                data:{'s1': data.value},
                type: "post",
                //async:false,
        ...
        form.on('select(s2)', function(data){
            $("#s3").empty();
            $.ajax({
                url:"{:U('Index/index',array('show' => 'searchCity'))}",
                data:{'s2': data.value},
                type: "post",
                //async:false,
                success: function (json) {
