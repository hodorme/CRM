...
window.ShearPhoto.MINGGE(function() {
	//██████████重要设置████████████████
	var relativeUrl = "/Public/Addons/shearphoto_common/"; //你不要在后面加斜杠，系统会自动给你加上斜杠，不信看下面！   index.html的JS引用路径自己改，很简单的说
	/*
	...
	Shear.config({
		...
		scopeWidth: 350, //可拖动范围宽  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致  

		scopeHeight: 400, //可拖动范围高  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致      

		proportional: [1/1,
		<!--截框的宽高比例（宽除以高的比例值，这个设置其实就是1，对！你可以直接写1  如填3/4 那么就是0.75的比例,不设比例请设为0，注意更改比例后，后端也要进行相应设置，否则系统会给你抱出错误-->
			/*
	...
	try {
		webcam.init_config = {
				...
				width: 1000, //摄像头像素宽度，也就是说后端生成图片的大小

				height: 1000, //摄像头像素高度，也就是说后端生成图片的大小

				server_width: 350, //网页中flash的显示宽度

				server_height: 350, //网页中flash的显示高度
